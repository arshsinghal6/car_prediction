...
while j < 10:
    ...
    # print(j)
    # print(type(src_pts) == type(dst_pts))
    # print(src_pts.shape, dst_pts.shape)

    # # Compute the essential matrix (E)
    # E, _ = cv2.findEssentialMat(src_pts, dst_pts, intrinsic_matrix)

    # # Recover camera pose (rotation and translation) from the essential matrix (E)
    # _, R, t, _ = cv2.recoverPose(E, src_pts, dst_pts, intrinsic_matrix)

    # yaw_rad = np.arctan2(R[1, 0], R[0, 0])
    # pitch_rad = np.arctan2(-R[2, 0], np.sqrt(R[2, 1]**2 + R[2, 2]**2))

    # print("YAW " + str(i) + ": " + str(yaw_rad))
    # print("PITCH " + str(i) + ": " + str(pitch_rad))
